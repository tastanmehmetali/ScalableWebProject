...
import static com.waes.assignment.util.UtilDiffProcessor.generateRightEndpointForGivenId;
...
public class DiffRestController_SaveRightValueShould {
	...
	public void testCreatedDiffDataForGivenIdIsEmpty() throws Exception {
		mockDiffMvc.perform(post(generateRightEndpointForGivenId(StringUtils.EMPTY)).content(getSampleContentData()))
				.andExpect(status().isMethodNotAllowed());
	}

	@Test
	public void testCreatedDiffDataForGivenIdIsNull() throws Exception {
		mockDiffMvc.perform(post(generateRightEndpointForGivenId(null)).content(getSampleContentData()))
				.andExpect(status().isOk());
	}

	@Test
	public void testCreatedDiffDataForGivenValidValues() throws Exception {
		mockDiffMvc.perform(post(generateRightEndpointForGivenId(ID_1)).content(getSampleContentData()))
				.andExpect(status().isOk());
	}

	@Test
	public void testCreatedDiffDataWhenNotIncludeContent() throws Exception {
		mockDiffMvc.perform(post(generateRightEndpointForGivenId(ID_1))).andExpect(status().is4xxClientError());
	}

	@Test
	public void testCreatedDiffDataWhenIncludeContentIsEmpty() throws Exception {
		mockDiffMvc.perform(post(generateRightEndpointForGivenId(ID_1)).content(StringUtils.EMPTY))
				.andExpect(status().is4xxClientError());
	}

	@Test
	public void testCreatedDiffDataWhenIncludeContentIsNull() throws Exception {
		mockDiffMvc.perform(post(generateRightEndpointForGivenId(ID_1)).content(new byte[0]))
				.andExpect(status().is4xxClientError());
	}

	@Test
	public void testClientErrorWhenContentNotValidBase64() throws Exception {
		mockDiffMvc.perform(post(generateRightEndpointForGivenId(ID_1)).content(GIVEN_JSON_SAMPLE_VALUE))
				.andExpect(status().is4xxClientError());
	}

	@Test
	public void testClientErrorWhenContentByteNotValidBase64() throws Exception {
		mockDiffMvc.perform(post(generateRightEndpointForGivenId(ID_1)).content(GIVEN_JSON_SAMPLE_VALUE.getBytes()))
				.andExpect(status().is4xxClientError());
	}
}
